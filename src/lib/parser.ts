...
async function parse(pathname: string) {
  ...
  // parseDefaultFunction(sourceFile.statements)
...
function parseDefaultFunction(statements: NodeArray<Statement>): {
  ...
  const functionParams = getFunctionParams(functionDeclaration.parameters)
...
function getFunctionParams(funcParams: NodeArray<ParameterDeclaration>): FunctionParams[] {
